    ...
    <div className="min-h-screen bg-slate-50 pt-32 pb-24 relative overflow-hidden font-sans text-slate-900">
      ...
      <div className="max-w-lg mx-auto px-6 relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-3xl bg-white border border-slate-200 text-amber-500 mb-6 shadow-sm overflow-hidden p-2">
             <img src="/machakos-logo.png" alt="Logo" className="w-full h-full object-contain" onError={(e) => { e.currentTarget.style.display = 'none'; const fallback = document.getElementById('machakos-fallback'); if (fallback) fallback.style.display = 'block'; }} />
             <Crown id="machakos-fallback" className="w-8 h-8 hidden" />
          </div>
          <h1 className="font-serif text-3xl md:text-5xl font-semibold text-slate-900 mb-4 tracking-tight">Mr & Miss Machakos</h1>
          <p className="text-slate-600 text-sm md:text-base leading-relaxed">
            Register below to secure your audition spot for the most prestigious pageant and leadership platform in the county.
          </p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-xl border border-slate-200 p-6 md:p-8 rounded-3xl shadow-xl relative overflow-hidden">
          ...
          {success ? (
            <div className="flex flex-col items-center text-center py-10 animate-in zoom-in-95 duration-500">
              ...
              <h2 className="font-serif text-3xl font-semibold text-slate-900 mb-3">Application Received!</h2>
              <p className="text-slate-600 mb-8 text-sm">
                Thank you for applying. Our team will review your application and contact you with audition details.
              </p>
              <Link href="/nominees" className="bg-slate-900 hover:bg-slate-800 text-white px-8 py-3.5 rounded-full font-bold text-xs uppercase tracking-widest transition-colors shadow-md">
                View Registered Nominees
              </Link>
            </div>
          ) : (
